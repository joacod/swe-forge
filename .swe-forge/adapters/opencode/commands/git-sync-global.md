...
its `sync` action only. Identify the PR for the current task branch and verify
the provider reports `MERGED` before changing the checkout; a `merged` argument
or user statement is a request to check, not proof. Confirm that the working tree
is clean, identify the remote default branch, fetch the remote, switch safely,
and run a fast-forward-only pull. Stop on an unmerged, missing, ambiguous, or
