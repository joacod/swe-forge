...
already open; otherwise generate a short imperative title and concise
summary/validation/risk body using the policy's format, then create the PR with
the host's supported CLI. Never merge, approve, close, force-push, or switch to
the default branch as a side effect.
